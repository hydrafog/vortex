...
- Android writes each decrypted 60 KB frame straight to `MediaStore.Downloads`.
...
- With auto-accept on in settings, files from a paired device save directly to Downloads. On NixOS and Home Manager, Vortex replaces the read-only store symlink with a user file first, so the setting sticks.
