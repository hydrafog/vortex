...
- **Folder Packaging**: Desktop client packages folder trees into high-throughput zip archive containers (`STORED`) using 1MB buffered I/O, streaming entire directory structures at maximum NVMe/SSD and local Wi-Fi speeds without single-threaded CPU compression bottlenecks. The receiver unpacks the tree directly into Downloads.
...
- **Real-Time Progress Notifications**: Receiving transfers on Android displays an ongoing progress notification indicating current file name, index progress (`X of Y`), byte counts, and a progress bar, updated with a 250ms debounce to prevent notification channel flooding.
