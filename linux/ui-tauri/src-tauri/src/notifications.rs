...
pub(crate) static NOTIF_SHOW: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(true);

pub(crate) static NOTIF_SEND: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(true);
...
pub(crate) async fn prompt_reply_text(prompt: &str) -> Option<String> {
    ...
    let candidates: [(&str, Vec<String>); 2] = [
        ("zenity", vec!["--entry".into(), format!("--title={title}"), format!("--text={prompt}")]),
        ("kdialog", vec!["--title".into(), title.into(), "--inputbox".into(), prompt.into()]),
    ];
...
pub(crate) fn spawn_subsystem(
    ...
    let catch_up_last = Arc::new(tokio::sync::Mutex::new(None::<std::time::Instant>));
    const CATCH_UP_MIN_INTERVAL: std::time::Duration = std::time::Duration::from_secs(90);
            ...
            while let Some(notif) = ble_notif_rx.recv().await {
                ...
                if notif.dismiss {
                    let id = {
                        ...
                        let found = m.iter().find(|(_, v)| v.0 == notif.key).map(|(&id, _)| id);
                ...
                match vortex_l3_daemon::core::notification_display::show(&notif, replaces_id).await
                {
                    Ok(id) => {
        ...
        tokio::spawn(async move {
            while let Some((id, reason)) = closed_rx.recv().await {
                ...
                tokio::spawn(async move {
                    ...
                    let Some((key, _reply_index, shown_at, _title, _app_id, _app_label)) = link
                    else {
                        return;
                    };
                    if was_action {
        ...
        tokio::spawn(async move {
            while let Some((id, action_key)) = act_rx.recv().await {
                ...
                tokio::spawn(async move {
                    ...
                    let seq = NOTIF_INVOKE_SEQ.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
